 ...
 *this function
 *
 *refer to: https://stackoverflow.com/questions/122616/how-do-i-trim-leading-trailing-whitespace-in-a-standard-way
 *
 *this function bascially delete all the trailing spaces from input argument  &str
 *
 */
...
//WHAT IF WE enter a file name can't open
	...
	char **instream = malloc(sizeof(char*)*10);
        for(int i = 0; i< 10; i++) instream[i]= malloc(40*sizeof(char));
		...
		switch(sit)
		{
			...
			case 'b':
				...
				temp = InsertSpace(string,len,pos);	
			       	temp2 = InsertSpace(temp,len+1,pos+2);
	...
	if(redict == 1)
	{
		...
		if(size_input >= 3 && strcmp(instream[size_input-2],">")== 0)
		{
			...
			//for(int x=0; x<size_input -3; x++) printf("argv:%s\n",argv[x]);
		}
		else
		{
			error();
