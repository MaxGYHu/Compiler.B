...
def path(close):
    ...
    while True:
        ...
        if next_node == None:
            break
        #add the key, the first and second element from its value
...
def solve(state):
    ...
    while final:
        ...
        sucors = succ(node)
        for i in sucors:
            #if it is not in the close list
            #then add it into the final
