    ...
    for point1 in cluster1:
        for point2 in cluster2:
            ...
            if d < small:
                ...
                num1 = label.index(point1)
                num2 = label.index(point2)
    #print(small)
       ...
       #update the 20
        if len(valid_data[pos1]) > 1:
            for j in range(len(new_cluster)):
                if ele1 in new_cluster[j][1]:
                      index1 = new_cluster[j][0]
            
        if len(valid_data[pos2]) > 1:
            for j in range(len(new_cluster)):
                if ele2 in new_cluster[j][1]:
...
def imshow_hac(dataset):
    ...
    while cluster[-1][3] != 20:  # when there are more clusters
        ...
        x_val = [el[0] for x in valid_data for el in x]
        y_val = [el[1] for x in valid_data for el in x]
